...
class simulation():
    ...
    def __init__(self, g1, g2, g3, beta, omega, gamma, mu, v_step, mode, waning_time, vacc_death_prob, links, vacc_start, vacc_limit):
        ...
        nodes_array = np.array(g2.nodes())
        nodes_allocation_dict = dict(zip(nodes_array, nodes_array + int(nodes_array.shape[0])))
        g3 = nx.relabel_nodes(g3, nodes_allocation_dict)
        g2 = nx.compose(g2, g3)
        
        nodes1 = np.random.choice(nodes_array, links)
        nodes2 = np.random.choice(nodes_array + int(nodes_array.shape[0]), links)

        for i in range(links):
            g2.add_edge(nodes1[i], nodes2[i])
    ...
    def init_states(self, p0_inf, p0_op_minus, p0_op_plus):
        ...
        self.opinion_state = np.zeros(self.N)
        
        minus_opinion_nodes = np.random.choice(int(self.N/2), int(p0_op_minus*self.N), replace= False)
        plus_opinion_nodes = np.random.choice(int(self.N/2), int(p0_op_plus*self.N), replace= False) + int(self.N/2)
        
        for minus_node in minus_opinion_nodes:
            
            self.opinion_state[self.nodes_allocation_dict[minus_node]] = -1
            
        for plus_node in plus_opinion_nodes:
            
            self.opinion_state[self.nodes_allocation_dict[plus_node]] = 1
