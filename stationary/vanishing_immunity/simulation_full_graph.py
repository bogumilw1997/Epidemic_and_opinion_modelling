...
class simulation():
    ...
    def __init__(self, g1, beta, omega, gamma, mu, v_step, mode, waning_time, vacc_death_prob, vacc_start, vacc_limit):
        ...
        self.opinion_connections = np.arange(self.N)
    ...
    def check_opinion(self, node):
        ...
        if self.new_epidemic_state[node] != 'D':
            ...
            if self.n_count > 0:
                ...
                m_i = (self.opinion_state_sum - current_state)/self.n_count
                ...
                if current_state == -1:
                    ...
                    if self.new_epidemic_state[node] == "I":
                        i_count = self.i_count - 1
                    else:
                        i_count = self.i_count
                    
                    S_i = m_i + self.omega * i_count/self.n_count
    ...
    def do_one_sim_step(self, step):
        ...
        for _ in range(self.v_step):
            ...
            self.n_count = self.N - np.sum(self.new_epidemic_state == 'D') - 1
            self.opinion_state_sum = np.sum(self.opinion_state[self.new_epidemic_state != 'D'])
            self.i_count = np.sum(self.new_epidemic_state == 'I')
